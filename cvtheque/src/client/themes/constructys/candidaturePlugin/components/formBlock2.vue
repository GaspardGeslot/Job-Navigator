<template>
  <div class="formBlockLayout" @submit.prevent="onSubmit">
    ...
    <form v-if="sectors" class="formBlock5">
      ...
      <p class="formSubTitle">3 choix maximum</p>
      <div v-for="(item, itemIndex) in sectors" :key="itemIndex">
        ...
        <div
          ...
          style="
            ...
            cursor: pointer;
          "
          @click="toggleShowMore(itemIndex)"
        >
          <button class="toggleShowMoreButton" type="button">
            {{ showMore[itemIndex] ? 'Voir moins' : 'Voir d’autres métiers' }}
          </button>
          <svg
            v-if="showMore[itemIndex]"
            width="24"
            height="24"
            viewBox="0 0 24 24"
            fill="none"
