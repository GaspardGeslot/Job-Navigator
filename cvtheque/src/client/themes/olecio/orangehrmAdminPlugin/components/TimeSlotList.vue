          ...
          <oxd-form-row>
            <oxd-grid>
              <oxd-grid-item>
                <oxd-input-field
                  v-model="timeSlot"
                  :placeholder="$t('Nouveau créneau horaire')"
                />
              </oxd-grid-item>
            </oxd-grid>
          </oxd-form-row>
...
export default {
  ...
  setup() {
    ...
    const http = new APIService(
      ...
      `${window.appGlobal.theme}/api/v2/admin/time-slot`,
    );
    ...
    const onClickValidate = () => {
      ...
      else if (state.items.find((item) => item.title === state.timeSlot))
        return error({title: 'Conflit', message: 'Ce titre existe déjà'});
      http
        .create({title: state.timeSlot})
        .then(() => {
          state.timeSlot = '';
