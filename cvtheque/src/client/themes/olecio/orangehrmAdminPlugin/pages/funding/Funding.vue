          ...
          <oxd-form-row>
            <oxd-grid>
              <oxd-grid-item>
                <oxd-input-field
                  v-model="funding"
                  :placeholder="$t('Nouveau financement')"
                />
              </oxd-grid-item>
            </oxd-grid>
          </oxd-form-row>
...
export default {
  ...
  setup() {
    ...
    const http = new APIService(
      ...
      `${window.appGlobal.theme}/api/v2/admin/funding`,
    );
    ...
    const fetchFundingData = () => {
      ...
      http
        ...
        .catch((err) => {
          error({
            title: 'Erreur',
            message: 'Impossible de récupérer les financements',
          });
        })
        .finally(() => {
    ...
    const onClickValidate = () => {
      if (state.funding === '') {
        return;
      } else if (state.items.find((item) => item.name === state.funding)) {
        return error({title: 'Conflit', message: 'Ce financement existe déjà'});
      }
      http
        .create({name: state.funding})
        .then(() => {
          state.funding = '';
        ...
        .catch((err) => {
          error({
            title: 'Erreur',
            message: "Impossible d'ajouter le financement",
          });
        });
    };
    ...
    const sort = () => {
      ...
      else state.items.sort((a, b) => b.name.localeCompare(a.name));
    };
