...
export default {
  ...
  setup() {
    ...
    const http = new APIService(
      ...
      `${window.appGlobal.theme}/api/v2/admin/status`,
    );
    ...
    const fetchStatusData = () => {
      ...
      http
        ...
        .catch((err) => {
          error({
            title: 'Erreur',
            message: 'Impossible de récupérer les statuts',
          });
        })
        .finally(() => {
    ...
    const onClickValidate = () => {
      if (state.status === '') {
        return;
      } else if (state.items.find((item) => item.title === state.status)) {
        return error({title: 'Conflit', message: 'Ce statut existe déjà'});
      }
      http
        .create({name: state.status})
        .then(() => {
          state.status = '';
        ...
        .catch((error) => {
          error({title: 'Erreur', message: "Impossible d'ajouter le statut"});
        });
    };
