          ...
          <oxd-form-row>
            <oxd-grid>
              <oxd-grid-item>
                <oxd-input-field
                  v-model="handicap"
                  :placeholder="$t('Nouveau handicap')"
                />
              </oxd-grid-item>
            </oxd-grid>
          </oxd-form-row>
...
export default {
  ...
  setup() {
    ...
    const http = new APIService(
      ...
      `${window.appGlobal.theme}/api/v2/admin/handicap`,
    );
    ...
    const fetchHandicapData = () => {
      ...
      http
        ...
        .catch((err) => {
          error({
            title: 'Erreur',
            message: 'Impossible de récupérer les handicaps',
          });
        })
        .finally(() => {
    ...
    const onClickValidate = () => {
      if (state.handicap === '') {
        return;
      } else if (state.items.find((item) => item.title === state.handicap)) {
        return error({title: 'Conflit', message: 'Cet handicap existe déjà'});
      }
      http
        .create({name: state.handicap})
        .then(() => {
          state.handicap = '';
        ...
        .catch((error) => {
          error({title: 'Erreur', message: "Impossible d'ajouter l'handicap"});
        });
    };
