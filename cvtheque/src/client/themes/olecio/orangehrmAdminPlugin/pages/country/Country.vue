...
export default {
  ...
  setup() {
    ...
    const http = new APIService(
      ...
      `${window.appGlobal.theme}/api/v2/admin/countries`,
    );
    ...
    const fetchCourseStartData = () => {
      ...
      http
        ...
        .catch((err) => {
          error({title: 'Erreur', message: 'Impossible de récupérer les pays'});
        })
        .finally(() => {
    ...
    const onClickValidate = () => {
      if (state.courseStart === '') {
        return;
      } else if (state.items.find((item) => item.title === state.courseStart)) {
        return error({title: 'Conflit', message: 'Ce pays existe déjà'});
      }
      http
        ...
        .catch((error) => {
          error({title: 'Erreur', message: "Impossible d'ajouter le pays"});
        });
    };
