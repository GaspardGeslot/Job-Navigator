...
export default {
  ...
  setup() {
    ...
    const http = new APIService(
      ...
      `${window.appGlobal.theme}/api/v2/admin/need`,
    );
    ...
    const fetchNeedData = () => {
      ...
      http
        ...
        .catch((err) => {
          error({
            title: 'Erreur',
            message: 'Impossible de récupérer les besoins',
          });
        })
        .finally(() => {
    ...
    const onClickValidate = () => {
      if (state.need === '') {
        return;
      } else if (state.items.find((item) => item.title === state.need)) {
        return error({title: 'Conflit', message: 'Ce besoin existe déjà'});
      }
      http
        .create({name: state.need})
        .then(() => {
          state.need = '';
        ...
        .catch((error) => {
          error({title: 'Erreur', message: "Impossible d'ajouter le besoin"});
        });
    };
