          ...
          <oxd-form-row>
            <oxd-grid>
              <oxd-grid-item>
                <oxd-input-field
                  v-model="phone"
                  :placeholder="$t('Nouveau numéro de téléphone')"
                />
              </oxd-grid-item>
            </oxd-grid>
          </oxd-form-row>
...
export default {
  ...
  setup() {
    ...
    const http = new APIService(
      ...
      `${window.appGlobal.theme}/api/v2/admin/phone`,
    );
    ...
    const fetchPhoneData = () => {
      ...
      http
        ...
        .catch((err) => {
          error({
            title: 'Erreur',
            message: 'Impossible de récupérer les numéros de téléphone',
          });
        })
        .finally(() => {
    ...
    const onClickValidate = () => {
      if (state.phone === '') {
        return;
      } else if (state.items.find((item) => item.title === state.phone)) {
        return error({
          title: 'Conflit',
      ...
      http
        ...
        .catch((error) => {
          error({
            title: 'Erreur',
            message: "Impossible d'ajouter le numéro de téléphone",
          });
        });
    };
