...
class FundingController extends AbstractVueController
{
    use AuthUserTrait;
    ...
    {
        $component = new Component('funding-list');
    ...
    public function add(Request $request)
    {
        ...
        $this->addFunding($data['name'], $this->getAuthUser()->getUserHedwigeToken());
        return new Response(
            null,
            Response::HTTP_OK,
            ['Content-Type' => 'application/json']
    ...
    {
        $data = json_decode($request->getContent(), true);
        $this->deleteFunding($data['name'], $this->getAuthUser()->getUserHedwigeToken());
        return new Response(
            null,
            Response::HTTP_OK,
            ['Content-Type' => 'application/json']
    ...
    private function getFundings(string $token): array
    {
        ...
        try {
            ...
            $transformedData = array_map(function($funding) {
                return ['name' => $funding];
            }, $data);
    ...
    private function addFunding(string $name, string $token)
    {
        $client = new Client();
        $clientBaseUrl = getenv('HEDWIGE_URL');
        $url = "{$clientBaseUrl}/funding";
        try {
            ...
            error_log('error add funding: ' . $e->getMessage());
        }
    }

    private function deleteFunding(string $name, string $token)
    {
        $client = new Client();
        $clientBaseUrl = getenv('HEDWIGE_URL');
        $url = "{$clientBaseUrl}/funding";
        try {
            ...
            error_log('error delete funding: ' . $e->getMessage());
        }
    }
} 